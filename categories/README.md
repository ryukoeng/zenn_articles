# 記事カテゴリ index

このフォルダは `articles/` の記事を分類別に管理する index。**Zenn の公開対象外**（公開されるのは `articles/` と `books/` のみ）なので、URL や公開には影響しない。

新規記事を作成したら、該当カテゴリの `.md` に1行追記する（→ `zenn-article` スキル）。1記事が複数カテゴリにまたがる場合は、各ファイルに載せてよい。

| カテゴリ | ファイル | 内容 |
| ---- | ---- | ---- |
| AWS | [aws.md](aws.md) | EC2 / S3 / VPC など AWS 関連 |
| ネットワーク | [network.md](network.md) | IPアドレス / TCP/IP / サブネットなど |
| Web / HTTP | [web.md](web.md) | HTTP ヘッダーなど Web 関連 |

`published` 列: 公開済み（`published: true`） / 下書き（`published: false`）
