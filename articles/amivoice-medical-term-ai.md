...
*音声をアップロードして文字起こし。専門用語が黄色くハイライトされ、クリックで右側に解説が出る*

---

## TL;DR

- **AmiVoice の `-a-medical`（会話_医療）エンジン**で医療会話を文字起こしし、レスポンスの **`tokens[]`（単語単位）** を使って用語をハイライトする。`written`（表記）を照合キー、`confidence`（信頼度）を「聞き取りが不確実な箇所」のマークに使うのがこのアプリの肝。
...
加えてエンジンに **`-a-medical`（会話_医療）** を選べる点が大きい。病名・薬品名・手術名・病院名といった医療語に強く、汎用エンジンより表記の崩れが少なくなります。「単語に区切られている」×「医療語に強い」が揃うことで、用語ハイライトという体験が成立します。
  ...
  "results": [
    {
      ...
      "text": "アドバンスト・メディアは豊かな未来を目指します",
      "tokens": [
        { "written": "アドバンスト・メディア", "spoken": "あどばんすとめでぃあ",
          "confidence": 1, "starttime": 522, "endtime": 1578 }
      ],
      "tags": [], "rulename": ""
    }
  ]
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!appKey) {
    return Response.json({ error: "AMIVOICE_APPKEY が未設定です" }, { status: 500 });
  }
  ...
  if (file.size > MAX_BYTES) {
    return Response.json({ error: "ファイルが大きすぎます（16MBまで）" }, { status: 413 });
  }
  ...
  const res = await fetch(AMIVOICE_ENDPOINT, { method: "POST", body: upstream });
...
export function detectTerms(tokens: DetectToken[], glossaryKeys: string[]): Segment[] {
  ...
  // 最長一致のため長い順に並べる
  const keys = glossaryKeys.filter((k) => k.length > 0).sort((a, b) => b.length - a.length);
...
*辞書に無い「狭心症」もAIが解説。出所を明示してハルシネーションに注意を促す*
...
*アップロード前の初期状態。凡例で「低信頼」マークの意味を先に示す*
